...
export default function PostManager({ category, title, description }) {
    ...
    <div ref={pageRef} className="space-y-6 relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 admin-anim">
        ...
        >
          ...
          Tambah {title.replace("Manajemen ", "")}
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {posts.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center text-mute text-sm">Belum ada data.</td>
                </tr>
              ) : (
                posts.map(post => (
                  <tr key={post.id} className="hover:bg-surface-soft/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Link 
                          href={`/publikasi/${category}/${post.id}`} 
                          target="_blank"
                        ...
                        <button onClick={() => handleDeleteClick(post.id)} className="p-2 text-mute hover:text-accent bg-surface-soft hover:bg-accent/10 rounded-xl transition-colors" title="Hapus">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-canvas rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto relative z-10 shadow-2xl p-6 md:p-8 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-ink">{editingId ? `Edit ${title.replace("Manajemen ", "")}` : `Tambah ${title.replace("Manajemen ", "")}`}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-semibold text-ink-soft mb-2">Judul</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                >
                  {isSubmitting ? 'Menyimpan...' : (editingId ? 'Simpan Perubahan' : 'Terbitkan')}
                </button>
              </div>
            </form>
          </div>
